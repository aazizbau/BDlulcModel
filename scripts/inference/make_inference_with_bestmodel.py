...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--ae``, ``--indices-dir``, ``--checkpoint``, ``--train-npz``, ``--output-root``, ``--device``, ``--batch-size``, ``--save-probability-stack``. Run the ``--help`` command below for required values, defaults, and accepted choices.
