...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--best-runs-csv``, ``--output-root``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
Inspect the complete interface before supplying AOI-specific paths::
    ...
    python scripts/spatial_block_bootstrap/preparation/01_identify_selected_runs.py --help
"""
