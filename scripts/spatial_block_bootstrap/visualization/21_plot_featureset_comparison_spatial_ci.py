#!/usr/bin/env python3
"""Plot AE64 versus AE64 plus indices with paired spatial confidence intervals.
...
Inspect the complete interface before supplying AOI-specific paths::
    ...
    python scripts/spatial_block_bootstrap/visualization/21_plot_featureset_comparison_spatial_ci.py --help
"""
...
from common.plot_utils import TEXT_EFFECTS, add_ci_labels, asymmetric_yerr  # noqa: E402
...
def main() -> None:
    ...
    output = resolve_path(args.output_plot) if args.output_plot else root / "figures" / "featureset_comparison_spatial_block_ci.png"
    output.parent.mkdir(parents=True, exist_ok=True)
    summary = pd.read_csv(root / "summaries" / "featureset_comparison_spatial_ci.csv").set_index("model_family").loc[MODEL_FAMILY_ORDER]
    ...
    for index, row in enumerate(summary.itertuples()):
        ...
        ax.text(
            index,
            annotation_y,
            f"Delta {row.observed_delta:+.2f} pp\n95% CI: {row.delta_lower_95:+.2f} to {row.delta_upper_95:+.2f}",
            ha="center",
            va="bottom",
            fontsize=7.5,
    ...
    if args.add_title:
        ...
        ax.set_title(
            "AE64 versus AE64 + 10 Spectral Indices\n"
