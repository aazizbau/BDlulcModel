...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--output-root``, ``--add-title``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
Inspect the complete interface before supplying AOI-specific paths::
    ...
    python scripts/spatial_block_bootstrap/visualization/23_create_spatial_bootstrap_tables.py --help
"""
