#!/usr/bin/env python3
"""Plot best-model class-wise metrics with spatial block bootstrap intervals.
...
Inspect the complete interface before supplying AOI-specific paths::
    ...
    python scripts/spatial_block_bootstrap/visualization/22_plot_bestmodel_classwise_spatial_ci.py --help
"""
...
def main() -> None:
    ...
    output = resolve_path(args.output_plot) if args.output_plot else root / "figures" / "bestmodel_classwise_spatial_block_ci.png"
    ...
    if args.add_title:
        ...
        ax.set_title(
            "Class-wise Accuracy Metrics for the Validation-Selected Best Model\n"
            f"{first['model_family']} | {first['feature_set']} | spatial block bootstrap 95% confidence intervals",
            pad=14,
        )
    fig.tight_layout()
