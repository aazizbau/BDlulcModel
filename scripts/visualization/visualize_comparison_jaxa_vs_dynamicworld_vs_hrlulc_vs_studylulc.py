#!/usr/bin/env python3
"""
Create a 3 x 5 comparison figure for random AOIs:
- column 1: Sentinel-2 RGB 2023
- column 2: Dynamic World 2023 (Harmonized)
- column 3: JAXA HRLULC 2023 (Harmonized)
- column 4: HRLULC BGD 2023 (Harmonized)
- column 5: Study LULC 2023
...
- data/processed/manik_lulc/bd_coastal_manik_lulc_2023_clipped.tif
...
Output
------
- outputs/figures/comparison_jaxa_vs_dynamicworld_vs_hrlulc_vs_studylulc_2023.png

Example
-------
python scripts/visualization/visualize_comparison_jaxa_vs_dynamicworld_vs_hrlulc_vs_studylulc.py
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--b2-2023``, ``--b3-2023``, ``--b4-2023``, ``--dynamicworld``, ``--jaxa``, ``--hrlulc``, ``--study-lulc``, ``--output-fig``, ``--seed``, ``--window``, ``--window-width-px``, ``--window-height-px``, ``--max-attempts``, ``--n-rows``, ``--dpi``, ``--add-main-title``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
# Mapping from HRLULC_categories_2023BGD_v25.09_10m IDs to MY_LULC_NAMES IDs
HRLULC_TO_MY_LULC = {
    1: 8,   # Water -> Rivers & Estuarine Channels
    2: 1,   # Built-up -> Urban / Institutional Built-up
    3: 4,   # Single cropland -> Cropland (All Crop Intensities)
    4: 4,   # Multiple cropland -> Cropland (All Crop Intensities)
    5: 6,   # Aquaculture -> Aquaculture & Inland Ponds
    6: 5,   # Orchards -> Tree-based Agroforestry & Orchard
    7: 1,   # Brickfield -> Urban / Institutional Built-up (Industrial/Man-made)
    8: 5,   # Forest -> Tree-based Agroforestry & Orchard
    9: 9,   # Mangrove -> Mangrove Forest
    10: 10, # Salt pans -> Bare / Exposed Coastal Land
    11: 5,  # Rubber tree -> Tree-based Agroforestry & Orchard
    12: 4,  # Jhum -> Cropland (All Crop Intensities/Shifting cultivation)
    13: 10, # Bare land -> Bare / Exposed Coastal Land
    14: 5,  # Tea garden -> Tree-based Agroforestry & Orchard
}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--hrlulc", default="data/processed/manik_lulc/bd_coastal_manik_lulc_2023_clipped.tif")
    p.add_argument("--study-lulc", default="outputs/inference/2023/lulc_class_2023.tif")
    p.add_argument("--output-fig", default="outputs/figures/comparison_jaxa_vs_dynamicworld_vs_hrlulc_vs_studylulc_2023.png")
...
def harmonize_hrlulc(arr: np.ndarray, nodata_value: float | int | None) -> np.ndarray:
    out = np.zeros(arr.shape, dtype=np.uint8)
    valid = np.isfinite(arr)
    if nodata_value is not None:
        valid &= arr != nodata_value
    valid &= arr > 0
    for src_class, dst_class in HRLULC_TO_MY_LULC.items():
        out[valid & (arr == src_class)] = dst_class
    return out
...
def choose_random_window_for_comparison(
    ...
    hrlulc_view: rasterio.DatasetReader,
    width_px: int,
    height_px: int,
    ...
    for _ in range(max_attempts):
        ...
        hrlulc_arr = harmonize_hrlulc(hrlulc_view.read(1, window=window), hrlulc_view.nodata)
        valid = (study_arr > 0) & (dw_arr > 0) & (jaxa_arr > 0) & (hrlulc_arr > 0)
        disagreement = valid & (
            (study_arr != dw_arr) | (study_arr != jaxa_arr) | (study_arr != hrlulc_arr) |
            (dw_arr != jaxa_arr) | (dw_arr != hrlulc_arr) | (jaxa_arr != hrlulc_arr)
        )
...
def read_window_payload(
    ...
    hrlulc_h = harmonize_hrlulc(hrlulc_view.read(1, window=window), hrlulc_view.nodata)
    study_arr = study_ds.read(1, window=window)

    rgb_img = stretch_rgb(rgb_arr, b2_view.nodata)
    dw_img = lulc_rgb_map(dw_h, nodata_color)
    jaxa_img = lulc_rgb_map(jaxa_h, nodata_color)
    hrlulc_img = lulc_rgb_map(hrlulc_h, nodata_color)
    study_img = lulc_rgb_map(study_arr, nodata_color)

    valid_mask = (dw_h > 0) & (jaxa_h > 0) & (hrlulc_h > 0) & (study_arr > 0)
    disagreement_mask = valid_mask & (
        (dw_h != jaxa_h) | (dw_h != hrlulc_h) | (dw_h != study_arr) |
        (jaxa_h != hrlulc_h) | (jaxa_h != study_arr) | (hrlulc_h != study_arr)
    )
...
def main() -> None:
    ...
    with rasterio.open(input_paths[0]) as b2_src, \
        ...
        rasterio.open(input_paths[5]) as hrlulc_src, \
        rasterio.open(input_paths[6]) as study_src:
        ...
        hrlulc_view = aligned_view(hrlulc_src, study_src, Resampling.nearest)
    ...
    if args.add_main_title:
        fig.suptitle(
            "Sentinel-2 RGB, Dynamic World, JAXA HRLULC, HRLULC BGD, and Study LULC comparison (2023)",
            fontsize=20,
            y=0.995,
            fontweight="bold",
            color=colors["text"],
    ...
    fig.subplots_adjust(left=0.045, right=0.988, top=top_margin, bottom=0.14, wspace=0.08, hspace=0.30)
