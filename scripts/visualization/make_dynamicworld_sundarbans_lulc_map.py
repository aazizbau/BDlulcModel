#!/usr/bin/env python3
"""
Create a publication-style Sundarbans-only Dynamic World LULC map for a given year.
...
Output
------
- outputs/figures/sundarbans_dynamicworld_lulc_<year>.png

Example
-------
python scripts/visualization/make_dynamicworld_sundarbans_lulc_map.py --year 2023
python scripts/visualization/make_dynamicworld_sundarbans_lulc_map.py --year 2023 \
    --buffer-top 1000 --buffer-bottom 10000 --buffer-left 10000 --buffer-right 10000

Complete Example Run
--------------------
python scripts/visualization/make_dynamicworld_sundarbans_lulc_map.py \
    --year 2023 \
    --add-title \
    --outptut-plot outputs/figures/sundarbans_dynamicworld_lulc_2023.png \
    --buffer-top 1000 \
    --buffer-bottom 10000 \
    --buffer-left 10000 \
    --buffer-right 10000
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--input``, ``--sundarbans-map``, ``--north-arrow``, ``--palette``, ``--add-title``, ``--outptut-plot``, ``--output``, ``--buffer-top``, ``--buffer-bottom``, ``--buffer-left``, ``--buffer-right``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
DEFAULT_INPUT_ROOT = Path("data/processed/dynamicworld")
...
CLASS_NODATA = -1
MAP_TITLE_TEMPLATE = "Sundarbans Dynamic World LULC {year}"
...
def default_input_path(year: int) -> Path:
    return DEFAULT_INPUT_ROOT / f"bd_coastal_dynamicworld_{year}_mode_clipped.tif"


def default_output_path(year: int) -> Path:
    return DEFAULT_OUTPUT_ROOT / f"sundarbans_dynamicworld_lulc_{year}.png"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Make Sundarbans Dynamic World LULC map for a target year.")
    p.add_argument("--year", type=int, required=True, help="Target year, e.g. 2023.")
    p.add_argument("--input", type=Path, default=None, help="Optional Dynamic World GeoTIFF override.")
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Output PNG path. Default: outputs/figures/sundarbans_dynamicworld_lulc_<year>.png",
    )
...
def legend_handles() -> list[Patch]:
    return [
        Patch(facecolor=DW_COLORS[class_id], edgecolor="#314245", label=DW_NAMES[class_id])
