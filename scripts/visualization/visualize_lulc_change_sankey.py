...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--transition``, ``--zone-map``, ``--palette``, ``--output-dir``, ``--add-title``, ``--outptut-plot-overall``, ``--outptut-plot-western``, ``--outptut-plot-central``, ``--outptut-plot-eastern``, ``--output-csv``. Run the ``--help`` command below for required values, defaults, and accepted choices.
