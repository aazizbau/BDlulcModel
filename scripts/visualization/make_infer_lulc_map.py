...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--input``, ``--zone-map``, ``--sundarbans-map``, ``--north-arrow``, ``--palette``, ``--output``, ``--add-title``, ``--output-plot``, ``--output-csv``. Run the ``--help`` command below for required values, defaults, and accepted choices.
