#!/usr/bin/env python3
"""
Create a publication-style Sundarbans-only Bangladesh HRLULC map for a given year.
...
Output
------
- outputs/figures/sundarbans_bd_hrlulc_<year>.png
...
CLASS_NODATA = 0
MAP_TITLE_TEMPLATE = "Sundarbans BD HRLULC {year}"
...
def default_output_path(year: int) -> Path:
    return DEFAULT_OUTPUT_ROOT / f"sundarbans_bd_hrlulc_{year}.png"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Make Sundarbans BD HRLULC map for a target year.")
    p.add_argument("--year", type=int, required=True, help="Target year, e.g. 2023.")
    p.add_argument("--input", type=Path, default=None, help="Optional BD HRLULC GeoTIFF override.")
...
def main() -> None:
    ...
    with rasterio.open(input_raster) as ds:
        if ds.crs is None:
            raise ValueError("Input BD HRLULC raster has no CRS.")
        raster_crs = ds.crs
