...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--transition``, ``--sundarbans-map``, ``--palette``, ``--add-title``, ``--output-plot``, ``--output-csv``, ``--output-dir``. Run the ``--help`` command below for required values, defaults, and accepted choices.
