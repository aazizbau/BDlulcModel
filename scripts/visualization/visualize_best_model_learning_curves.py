...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--model-name``, ``--runs-root``, ``--master-history``, ``--palette``, ``--output``, ``--dpi``, ``--add-title``. Run the ``--help`` command below for required values, defaults, and accepted choices.
