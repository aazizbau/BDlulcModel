#!/usr/bin/env python3
"""
Create a publication-style Sundarbans-only inferred LULC map for a given year.
...
Output
------
- outputs/figures/sundarbans_infer_lulc_<year>.png
...
--------------------
python scripts/visualization/make_infer_sundarbans_lulc_map.py \
    --year 2017 \
    --add-title \
    --output-plot outputs/figures/sundarbans_infer_lulc_2017.png \
    --buffer-top 1000 \
    --buffer-bottom 15800 \
    --buffer-left 10000 \
    --buffer-right 10000
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--input``, ``--sundarbans-map``, ``--north-arrow``, ``--palette``, ``--add-title``, ``--output-plot``, ``--output``, ``--buffer-top``, ``--buffer-bottom``, ``--buffer-left``, ``--buffer-right``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
def default_output_path(year: int) -> Path:
    return DEFAULT_OUTPUT_ROOT / f"sundarbans_infer_lulc_{year}.png"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Make Sundarbans inferred LULC map for a target year.")
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Output PNG path. Default: outputs/figures/sundarbans_infer_lulc_<year>.png",
    )
...
def add_graticule(ax, color: str, src_crs) -> None:
    ...
    # Fewer bins suit the small Sundarbans extent (~100 km × ~100 km)
...
def read_clipped_class_raster(
    ...
    for row0 in range(0, dst_h, chunk_size):
        ...
        for col0 in range(0, dst_w, chunk_size):
            ...
            out[row0:row1, col0:col1] = tile.astype(np.uint8, copy=False)
...
def main() -> None:
    ...
    output = resolve_path(args.output_plot or args.output or default_output_path(args.year))
