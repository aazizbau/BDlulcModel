...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--input``, ``--zone-map``, ``--sundarbans-map``, ``--north-arrow``, ``--palette``, ``--add-title``, ``--output-plot``, ``--output-csv``, ``--total-csv``, ``--zonewise-csv``, ``--title``. Run the ``--help`` command below for required values, defaults, and accepted choices.
