...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--npz``, ``--background``, ``--outfig``, ``--north-arrow``, ``--seed``, ``--context-blocks``, ``--max-bg-size``, ``--dpi``, ``--title``, ``--add-main-title``, ``--no-grid``, ``--scalebar-a-m``, ``--scalebar-b-m``. Run the ``--help`` command below for required values, defaults, and accepted choices.
