...
-------------------------------
Workflow role: Create a publication or poster-oriented composite map from prepared outputs.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--seed``, ``--zoom-window-km``, ``--zoom-inset-x-frac``, ``--zoom-inset-y-frac``, ``--input``, ``--zone-map``, ``--sundarbans-map``, ``--north-arrow``, ``--palette``, ``--output``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Replace map layers and output paths only after regenerating all analytical products for the target AOI.
