#!/usr/bin/env python3
"""
Create BSI GeoTIFF from Sentinel-2 coastal solid rasters.
...
Expected default output file:
  data/interim/bdcoastal_solid_2017_utm46_bsi.tif

Example runs:
python scripts/s2_indices/make_bsi_image.py --year 2017

python scripts/s2_indices/make_bsi_image.py \
  --year 2017 \
  --crs EPSG:32646 \
  --output data/interim/bdcoastal_solid_2017_utm46_bsi.tif
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--input-dir``, ``--crs``, ``--blue``, ``--red``, ``--nir``, ``--swir``, ``--output``, ``--input-nodata``, ``--output-nodata``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Make BSI image from Sentinel-2 B2 (blue), B4 (red), B8 (NIR), and B11 (SWIR) rasters."
    )
...
def resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path, Path, Path]:
    ...
    out_path = args.output or (args.input_dir / f"bdcoastal_solid_{args.year}_utm46_bsi.tif")
    return blue_path, red_path, nir_path, swir_path, out_path
...
def compute_bsi_block(
    ...
    valid = np.isfinite(bsi)
    valid_count = int(valid.sum())

    out = np.full(bsi.shape, output_nodata, dtype=np.float32)
    out[valid] = bsi[valid]
...
def print_output_summary(out_path: Path) -> None:
    with rasterio.open(out_path) as src:
        ...
        if "BSI_MIN" in tags and "BSI_MAX" in tags:
            log(f"  Min/Max    : {tags['BSI_MIN']} / {tags['BSI_MAX']}")
        if "BSI_MEAN" in tags and "BSI_STD" in tags:
            log(f"  Mean/Std   : {tags['BSI_MEAN']} / {tags['BSI_STD']}")
...
def main() -> None:
    ...
    blue_path, red_path, nir_path, swir_path, out_path = resolve_paths(args)

    log("Starting BSI creation")
    ...
    if not blue_path.exists():
        raise SystemExit(f"ERROR: Blue band file not found: {blue_path}")
    ...
    with (
        ...
        if blue_src.count != 1 or red_src.count != 1 or nir_src.count != 1 or swir_src.count != 1:
            raise SystemExit("ERROR: Expected single-band rasters for B2, B4, B8, and B11.")

        if (
            blue_src.width != red_src.width
            or blue_src.height != red_src.height
            or blue_src.width != nir_src.width
            or blue_src.height != nir_src.height
            or blue_src.width != swir_src.width
            or blue_src.height != swir_src.height
        ):
            raise SystemExit(
                f"ERROR: Raster size mismatch. "
                f"B2={blue_src.width}x{blue_src.height}, "
                f"B4={red_src.width}x{red_src.height}, "
                f"B8={nir_src.width}x{nir_src.height}, "
                f"B11={swir_src.width}x{swir_src.height}"
            )

        if blue_src.crs != red_src.crs or blue_src.crs != nir_src.crs or blue_src.crs != swir_src.crs:
            raise SystemExit(
                f"ERROR: CRS mismatch. "
                f"B2={blue_src.crs}, B4={red_src.crs}, B8={nir_src.crs}, B11={swir_src.crs}"
            )
        ...
        if (
            not same_transform(blue_src.transform, red_src.transform)
            or not same_transform(blue_src.transform, nir_src.transform)
            or not same_transform(blue_src.transform, swir_src.transform)
        ):
            raise SystemExit("ERROR: B2, B4, B8, and B11 transforms differ. Align rasters first.")
        ...
        log("Writing BSI raster by blocks...")
        with rasterio.open(out_path, "w", **profile) as dst:
            for _, window in blue_src.block_windows(1):
                ...
                bsi, valid_count = compute_bsi_block(
                    blue=blue,
                    red=red,
                    nir=nir,
                    swir=swir,
                ...
                dst.write(bsi, 1, window=window)

        log("Computing BSI statistics block by block...")
        bsi_stats = compute_raster_stats_blockwise(
            out_path,
        ...
        with rasterio.open(out_path, "r+") as dst:
            ...
            dst.update_tags(
                AREA_OR_POINT="Area",
                INDEX_NAME="BSI",
                INDEX_FORMULA="((B11 + B4) - (B8 + B2)) / ((B11 + B4) + (B8 + B2))",
                INDEX_DESCRIPTION="Bare Soil Index",
                YEAR=str(args.year),
                ...
                BSI_MIN=bsi_stats["min"],
                BSI_MAX=bsi_stats["max"],
                BSI_MEAN=bsi_stats["mean"],
                BSI_STD=bsi_stats["std"],
            )

        log(f"BSI raster written successfully (valid pixels written: {total_valid_pixels})")
