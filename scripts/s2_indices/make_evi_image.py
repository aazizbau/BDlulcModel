#!/usr/bin/env python3
"""
Create EVI GeoTIFF from Sentinel-2 coastal solid rasters.
...
Expected default output file:
  data/interim/bdcoastal_solid_2017_utm46_evi.tif

Example runs:
python scripts/s2_indices/make_evi_image.py --year 2017

python scripts/s2_indices/make_evi_image.py \
  --year 2017 \
  --crs EPSG:32646 \
  --output data/interim/bdcoastal_solid_2017_utm46_evi.tif
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--input-dir``, ``--crs``, ``--blue``, ``--red``, ``--nir``, ``--output``, ``--scale``, ``--input-nodata``, ``--output-nodata``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Make EVI image from Sentinel-2 B2 (blue), B4 (red), and B8 (NIR) rasters."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional output EVI path.",
    )
    parser.add_argument(
        "--scale",
        type=float,
        default=DEFAULT_SCALE,
        help=f"Reflectance scale factor (default: {DEFAULT_SCALE}).",
    )
    parser.add_argument(
        "--input-nodata",
        type=int,
...
def resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path, Path]:
    ...
    out_path = args.output or (args.input_dir / f"bdcoastal_solid_{args.year}_utm46_evi.tif")
    return blue_path, red_path, nir_path, out_path
...
def compute_evi_block(
    ...
    out = np.full(evi.shape, output_nodata, dtype=np.float32)
    out[valid] = evi[valid]
...
def print_output_summary(out_path: Path) -> None:
    with rasterio.open(out_path) as src:
        ...
        if "EVI_MIN" in tags and "EVI_MAX" in tags:
            log(f"  Min/Max    : {tags['EVI_MIN']} / {tags['EVI_MAX']}")
        if "EVI_MEAN" in tags and "EVI_STD" in tags:
            log(f"  Mean/Std   : {tags['EVI_MEAN']} / {tags['EVI_STD']}")
...
def main() -> None:
    ...
    blue_path, red_path, nir_path, out_path = resolve_paths(args)

    log("Starting EVI creation")
    ...
    with (
        ...
        if blue_src.count != 1 or red_src.count != 1 or nir_src.count != 1:
            raise SystemExit("ERROR: Expected single-band rasters for B2, B4, and B8.")
        ...
        if blue_src.crs != red_src.crs or blue_src.crs != nir_src.crs:
            raise SystemExit(
                f"ERROR: CRS mismatch. B2={blue_src.crs}, B4={red_src.crs}, B8={nir_src.crs}"
            )
        ...
        if not same_transform(blue_src.transform, red_src.transform) or not same_transform(
            blue_src.transform, nir_src.transform
        ):
            raise SystemExit("ERROR: B2, B4, and B8 transforms differ. Align rasters first.")
        ...
        log("Writing EVI raster by blocks...")
        with rasterio.open(out_path, "w", **profile) as dst:
            for _, window in blue_src.block_windows(1):
                ...
                evi, valid_count = compute_evi_block(
                    blue=blue,
                    red=red,
                    nir=nir,
                ...
                dst.write(evi, 1, window=window)

        log("Computing EVI statistics block by block...")
        evi_stats = compute_raster_stats_blockwise(
            out_path,
        ...
        with rasterio.open(out_path, "r+") as dst:
            ...
            dst.update_tags(
                AREA_OR_POINT="Area",
                INDEX_NAME="EVI",
                INDEX_FORMULA="2.5 * (B8 - B4) / (B8 + 6*B4 - 7.5*B2 + 1)",
                INDEX_DESCRIPTION="Enhanced Vegetation Index",
                YEAR=str(args.year),
                ...
                REFLECTANCE_SCALE=str(args.scale),
                ...
                EVI_MIN=evi_stats["min"],
                EVI_MAX=evi_stats["max"],
                EVI_MEAN=evi_stats["mean"],
                EVI_STD=evi_stats["std"],
            )

        log(f"EVI raster written successfully (valid pixels written: {total_valid_pixels})")
