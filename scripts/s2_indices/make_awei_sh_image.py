...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--input-dir``, ``--crs``, ``--blue``, ``--green``, ``--nir``, ``--swir1``, ``--swir2``, ``--output``, ``--input-nodata``, ``--output-nodata``. Run the ``--help`` command below for required values, defaults, and accepted choices.
