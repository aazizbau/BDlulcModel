#!/usr/bin/env python3
"""
Create NDWI GeoTIFF from Sentinel-2 coastal solid rasters.
...
Expected default output file:
  data/interim/bdcoastal_solid_2017_utm46_ndwi.tif

Example runs:
python scripts/s2_indices/make_ndwi_image.py --year 2017

python scripts/s2_indices/make_ndwi_image.py \
  --year 2017 \
  --crs EPSG:32646 \
  --output data/interim/bdcoastal_solid_2017_utm46_ndwi.tif
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--input-dir``, ``--crs``, ``--green``, ``--nir``, ``--output``, ``--input-nodata``, ``--output-nodata``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Make NDWI image from Sentinel-2 B3 (green) and B8 (NIR) rasters."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional output NDWI path.",
    )
    parser.add_argument(
        "--input-nodata",
        type=int,
...
def resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    out_path = args.output or (args.input_dir / f"bdcoastal_solid_{args.year}_utm46_ndwi.tif")
    return green_path, nir_path, out_path
...
def compute_ndwi_block(
    ...
    nodata_mask = np.zeros(green.shape, dtype=bool)
    ...
    out = np.full(ndwi.shape, output_nodata, dtype=np.float32)
    out[valid] = ndwi[valid]
...
def print_output_summary(out_path: Path) -> None:
    with rasterio.open(out_path) as src:
        ...
        if "NDWI_MIN" in tags and "NDWI_MAX" in tags:
            log(f"  Min/Max    : {tags['NDWI_MIN']} / {tags['NDWI_MAX']}")
        if "NDWI_MEAN" in tags and "NDWI_STD" in tags:
            log(f"  Mean/Std   : {tags['NDWI_MEAN']} / {tags['NDWI_STD']}")
...
def main() -> None:
    ...
    green_path, nir_path, out_path = resolve_paths(args)

    log("Starting NDWI creation")
    ...
    with rasterio.open(green_path) as green_src, rasterio.open(nir_path) as nir_src:
        if green_src.count != 1 or nir_src.count != 1:
            raise SystemExit("ERROR: Expected single-band rasters for B3 and B8.")

        if green_src.width != nir_src.width or green_src.height != nir_src.height:
            raise SystemExit(
        ...
        if green_src.crs != nir_src.crs:
            raise SystemExit(f"ERROR: CRS mismatch. B3={green_src.crs}, B8={nir_src.crs}")

        if green_src.crs is None:
            raise SystemExit("ERROR: Input rasters have no CRS.")

        if green_src.crs.to_string() != args.crs:
            raise SystemExit(
                f"ERROR: Input CRS is {green_src.crs.to_string()} but expected {args.crs}."
            )

        if not same_transform(green_src.transform, nir_src.transform):
            raise SystemExit("ERROR: B3 and B8 transforms differ. Align rasters first.")
        ...
        with rasterio.open(out_path, "w", **profile) as dst:
            for _, window in green_src.block_windows(1):
                ...
                dst.write(ndwi, 1, window=window)

        log("Computing NDWI statistics block by block...")
        ndwi_stats = compute_raster_stats_blockwise(
            out_path,
        ...
        with rasterio.open(out_path, "r+") as dst:
            ...
            dst.update_tags(
                AREA_OR_POINT="Area",
                INDEX_NAME="NDWI",
                INDEX_FORMULA="(B3 - B8) / (B3 + B8)",
                INDEX_DESCRIPTION="Normalized Difference Water Index (McFeeters)",
                YEAR=str(args.year),
        ...
        log(f"NDWI raster written successfully (valid pixels written: {total_valid_pixels})")
