#!/usr/bin/env python3
"""
Create NDVI GeoTIFF from Sentinel-2 coastal solid rasters.
...
Expected default output file:
  data/interim/bdcoastal_solid_2017_utm46_ndvi.tif

Example runs:
python scripts/s2_indices/make_ndvi_image.py --year 2017

python scripts/s2_indices/make_ndvi_image.py \
  --year 2017 \
  --crs EPSG:32646 \
  --output data/interim/bdcoastal_solid_2017_utm46_ndvi.tif
...
def resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    out_path = args.output or (args.input_dir / f"bdcoastal_solid_{args.year}_utm46_ndvi.tif")
...
def compute_ndvi(
    ...
    valid &= np.isfinite(red_f)
    valid &= np.isfinite(nir_f)

    denom = nir_f + red_f
    valid &= denom != 0.0

    ndvi = np.full(red.shape, output_nodata, dtype=np.float32)
    ...
    with rasterio.open(red_path) as red_src, rasterio.open(nir_path) as nir_src:
        ...
        with rasterio.open(out_path, "r+") as dst:
            ...
            dst.update_tags(
                AREA_OR_POINT="Area",
                INDEX_NAME="NDVI",
                INDEX_FORMULA="(B8 - B4) / (B8 + B4)",
                INDEX_DESCRIPTION="Normalized Difference Vegetation Index",
                YEAR=str(args.year),
