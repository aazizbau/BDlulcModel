#!/usr/bin/env python3
"""
Create NIRv GeoTIFF from Sentinel-2 coastal solid rasters.
...
Expected default output file:
  data/interim/bdcoastal_solid_2017_utm46_nirv.tif

Example runs:
python scripts/s2_indices/make_nirv_image.py --year 2017

python scripts/s2_indices/make_nirv_image.py \
  --year 2017 \
  --crs EPSG:32646 \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Make NIRv image from Sentinel-2 B4 (red) and B8 (NIR) rasters."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional output NIRv path.",
    )
    parser.add_argument(
        "--input-nodata",
        type=int,
...
def resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    out_path = args.output or (args.input_dir / f"bdcoastal_solid_{args.year}_utm46_nirv.tif")
...
def compute_nirv_block(
    ...
    valid = np.isfinite(nirv)
    valid_count = int(valid.sum())

    out = np.full(nirv.shape, output_nodata, dtype=np.float32)
    out[valid] = nirv[valid]
...
def print_output_summary(out_path: Path) -> None:
    with rasterio.open(out_path) as src:
        ...
        if "NIRV_MIN" in tags and "NIRV_MAX" in tags:
            log(f"  Min/Max    : {tags['NIRV_MIN']} / {tags['NIRV_MAX']}")
        if "NIRV_MEAN" in tags and "NIRV_STD" in tags:
            log(f"  Mean/Std   : {tags['NIRV_MEAN']} / {tags['NIRV_STD']}")
    ...
    with rasterio.open(red_path) as red_src, rasterio.open(nir_path) as nir_src:
        ...
        with rasterio.open(out_path, "w", **profile) as dst:
            for _, window in red_src.block_windows(1):
                ...
                dst.write(nirv, 1, window=window)

        log("Computing NIRv statistics block by block...")
        nirv_stats = compute_raster_stats_blockwise(
            out_path,
        ...
        with rasterio.open(out_path, "r+") as dst:
            ...
            dst.update_tags(
                AREA_OR_POINT="Area",
                INDEX_NAME="NIRV",
                INDEX_FORMULA="((B8 - B4) / (B8 + B4)) * B8",
                INDEX_DESCRIPTION="Near-Infrared Reflectance of Vegetation",
                YEAR=str(args.year),
                ...
                NIRV_MIN=nirv_stats["min"],
                NIRV_MAX=nirv_stats["max"],
                NIRV_MEAN=nirv_stats["mean"],
                NIRV_STD=nirv_stats["std"],
            )

        log(f"NIRv raster written successfully (valid pixels written: {total_valid_pixels})")
