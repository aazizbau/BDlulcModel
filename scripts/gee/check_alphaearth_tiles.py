"""
Verify AlphaEarth embedding tile downloads for the Bangladesh coastal AOI.

Usage:
    python scripts/gee/check_alphaearth_tiles.py \
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--output``, ``--tile-width-km``, ``--tile-height-km``, ``--tile-overlap-km``, ``--project``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
def check_tiles(
    ...
    missing: list[Path] = []
    total = 0
    for row, col in iterate_tiles(BD_COASTAL_BBOX, geometry, tile_width_km, tile_height_km, overlap_km):
        total += 1
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.5,
        help="Tile width in kilometers (default: 30 km).",
    )
    parser.add_argument(
        "--tile-height-km",
        type=float,
        default=2.5,
        help="Tile height in kilometers (default: 30 km).",
    )
    parser.add_argument(
        "--tile-overlap-km",
        type=float,
        default=0.5,
        help="Overlap between tiles in kilometers (default: 2 km).",
    )
    parser.add_argument(
        "--project",
        type=str,
