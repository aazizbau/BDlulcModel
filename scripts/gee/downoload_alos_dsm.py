...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--aoi``, ``--output``, ``--scale``, ``--crs``, ``--tile-deg``, ``--timeout``, ``--overwrite``, ``--project``. Run the ``--help`` command below for required values, defaults, and accepted choices.
