...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--aoi``, ``--output``, ``--crs``, ``--scale``, ``--project``, ``--tile-deg``, ``--dry-run``. Run the ``--help`` command below for required values, defaults, and accepted choices.
