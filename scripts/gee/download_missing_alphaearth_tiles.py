...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--year``, ``--output``, ``--crs``, ``--scale``, ``--tile-width-km``, ``--tile-height-km``, ``--tile-overlap-km``, ``--project``. Run the ``--help`` command below for required values, defaults, and accepted choices.
