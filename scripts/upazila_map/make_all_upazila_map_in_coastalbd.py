...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--zone-map``, ``--sundarbans-map``, ``--north-arrow``, ``--upazila-vectors``, ``--output``, ``--add-title``. Run the ``--help`` command below for required values, defaults, and accepted choices.
