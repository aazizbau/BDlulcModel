#!/usr/bin/env python3
"""
Train an MLP classifier on AE64 samples stored in NPZ.
...
Example:
python scripts/training/train_mlp_ae64_from_npz.py \
  --data data/processed/training/ae64_samples_4upazila_2023_trainvaltest.npz \
  --outdir runs/mlp_ae64_h512-256_do02_lr1e3_bs4096_v1 \
  --hidden 512 256 \
  --dropout 0.2 \
  --batch-size 4096 \
  --epochs 100 \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train MLP classifier on AE64 NPZ.")
    p.add_argument(
        "--data",
        type=Path,
