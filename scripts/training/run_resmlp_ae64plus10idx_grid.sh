...
# ==============================================================================
# Purpose: Run resmlp ae64plus10idx grid.
...
#   bash scripts/training/run_resmlp_ae64plus10idx_grid.sh
#
...
# ============================================================
# ResMLP grid search runner for AE64 + 10 indices classification
# ============================================================
#
# Runs 10 experiments and writes:
#   - per-run logs:
#       logs/resmlp_ae64plus10idx_grid/<run_name>.log
#   - master comparison CSV:
#       runs/resmlp_ae64plus10idx_master_runs.csv
#
# Usage:
#   chmod +x scripts/training/run_resmlp_ae64plus10idx_grid.sh
...
#     * best validation macro F1 = 0.7347 at epoch 8
#     * validation improved until around epoch 8, then slowly degraded
#     * scheduler reductions did not recover better validation results
#     * train metrics kept improving, indicating mild overfitting
# ============================================================
...
TRAIN_SCRIPT="scripts/training/train_resmlp_ae64_plus10indices_from_npz.py"
LOG_DIR="logs/resmlp_ae64plus10idx_grid"
MASTER_CSV="runs/resmlp_ae64plus10idx_master_runs.csv"
...
# - best val macro F1 = 0.7347 at epoch 8
# - best test macro F1 = 0.7296
# - validation rises for several epochs, then plateaus/declines
# - current setup is already decent, so the grid should explore:
#   * slightly lower learning rates
#   * a bit more dropout / weight decay / label smoothing
#   * both smaller and deeper residual layouts
#   * modest batch-size variation
# ------------------------------------------------------------

run_one "resmlp_ae64plus10idx_h512-256_do02_lr1e3_bs4096_v1" \
  --hidden 512 256 \
  --dropout 0.2 \
  --batch-size 4096 \
  --epochs 100 \
...
run_one "resmlp_ae64plus10idx_h512-256_do025_lr8e4_bs4096_v2" \
  --hidden 512 256 \
  --dropout 0.25 \
  --batch-size 4096 \
  --epochs 100 \
...
run_one "resmlp_ae64plus10idx_h512-256_do03_lr7e4_bs4096_v3" \
  --hidden 512 256 \
  --dropout 0.3 \
  --batch-size 4096 \
  --epochs 110 \
...
run_one "resmlp_ae64plus10idx_h256-256_do02_lr1e3_bs4096_v4" \
  --hidden 256 256 \
  --dropout 0.2 \
  --batch-size 4096 \
  --epochs 100 \
...
run_one "resmlp_ae64plus10idx_h256-256-256_do025_lr8e4_bs4096_v5" \
  --hidden 256 256 256 \
  --dropout 0.25 \
  --batch-size 4096 \
  --epochs 110 \
...
run_one "resmlp_ae64plus10idx_h512-512-256_do025_lr8e4_bs4096_v6" \
  --hidden 512 512 256 \
  --dropout 0.25 \
  --batch-size 4096 \
  --epochs 110 \
...
run_one "resmlp_ae64plus10idx_h768-384_do03_lr7e4_bs4096_v7" \
  --hidden 768 384 \
  --dropout 0.3 \
  --batch-size 4096 \
  --epochs 110 \
...
run_one "resmlp_ae64plus10idx_h512-256_do025_lr8e4_bs2048_v8" \
  --hidden 512 256 \
  --dropout 0.25 \
  --batch-size 2048 \
  --epochs 100 \
...
run_one "resmlp_ae64plus10idx_h512-256_do02_lr8e4_bs8192_v9" \
  --hidden 512 256 \
  --dropout 0.2 \
  --batch-size 8192 \
  --epochs 100 \
...
run_one "resmlp_ae64plus10idx_h384-384-192_do03_lr6e4_bs4096_v10" \
  --hidden 384 384 192 \
  --dropout 0.3 \
  --batch-size 4096 \
  --epochs 120 \
...
master_csv = Path("runs/resmlp_ae64plus10idx_master_runs.csv")
