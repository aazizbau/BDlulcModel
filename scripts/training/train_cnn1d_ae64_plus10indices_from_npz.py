#!/usr/bin/env python3
"""
Train a 1D-CNN classifier on AE64 + 10 indices samples stored in NPZ.
...
- trains a 1D-CNN classifier with weighted cross-entropy
...
Example:
python scripts/training/train_cnn1d_ae64_plus10indices_from_npz.py \
  --data data/processed/training/ae64_plus10indices_samples_4upazila_2023_trainvaltest.npz \
  --outdir runs/cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h128_do02_lr1e3_bs4096_v1 \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.2 \
...
- Expected input_dim is 74 for AE64 + 10 indices features.
- The 1D-CNN treats the 74 input features as a 1D sequence of length 74.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--data``, ``--outdir``, ``--channels``, ``--kernels``, ``--head-dim``, ``--dropout``, ``--batch-size``, ``--epochs``, ``--lr``, ``--weight-decay``, ``--patience``, ``--min-delta``, ``--label-smoothing``, ``--scheduler``, ``--scheduler-factor``, ``--scheduler-patience``, ``--eval-every``, ``--device``, ``--seed``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train 1D-CNN classifier on AE64 + 10 indices NPZ.")
    p.add_argument(
        "--data",
        type=Path,
    ...
    p.add_argument(
        ...
        nargs="+",
        default=[32, 64, 128],
        help="Conv channel sizes, e.g. --channels 32 64 128",
    )
    p.add_argument(
        "--kernels",
        type=int,
        nargs="+",
        default=[5, 3, 3],
        help="Kernel sizes for conv layers. Must match number of --channels entries.",
    )
    p.add_argument(
        "--head-dim",
        type=int,
        default=128,
        help="Hidden dimension of classifier head after global pooling (default: 128).",
    )
    p.add_argument(
        "--dropout",
        type=float,
...
class CNN1DClassifier(nn.Module):
    def __init__(
        ...
        for out_ch, k in zip(channels, kernels):
            if k <= 0 or k % 2 == 0:
                raise ValueError(f"Each kernel size must be a positive odd integer. Got {k}.")
            padding = k // 2
            conv_blocks.extend(
                [
                    nn.Conv1d(in_ch, out_ch, kernel_size=k, padding=padding, bias=False),
...
def main() -> None:
    ...
    if len(args.channels) != len(args.kernels):
        raise SystemExit("--channels and --kernels must have the same number of values.")
    if len(args.channels) == 0:
        raise SystemExit("--channels must contain at least one value.")
    if any(k <= 0 or k % 2 == 0 for k in args.kernels):
        raise SystemExit("All kernel sizes must be positive odd integers.")
    ...
    if input_dim != EXPECTED_INPUT_DIM:
        log(f"Warning: expected {EXPECTED_INPUT_DIM} features for AE64 + 10 indices, found {input_dim}")
    ...
    log(f"Channels          : {args.channels}")
    log(f"Kernels           : {args.kernels}")
    log(f"Head dim          : {args.head_dim}")
