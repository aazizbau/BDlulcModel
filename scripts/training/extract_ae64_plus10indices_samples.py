...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--ae``, ``--year``, ``--interim-dir``, ``--upazilas``, ``--output``, ``--max-per-class``, ``--val-frac``, ``--block-size-m``, ``--seed``, ``--label-nodata``, ``--ae-nodata``, ``--index-nodata``, ``--chunk``. Run the ``--help`` command below for required values, defaults, and accepted choices.
