...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--data``, ``--run-name``, ``--out-dir``, ``--epochs``, ``--batch-size``, ``--lr``, ``--weight-decay``, ``--hidden``, ``--depth``, ``--dropout``, ``--no-bn``, ``--label-smoothing``, ``--focal-gamma``, ``--focal-alpha``, ``--tau``, ``--tau-list``, ``--seed``, ``--seeds``, ``--ensemble-eval``, ``--early-stop-patience``, ``--plateau-patience``, ``--plateau-factor``, ``--min-lr``, ``--amp``, ``--tensorboard``. Run the ``--help`` command below for required values, defaults, and accepted choices.
