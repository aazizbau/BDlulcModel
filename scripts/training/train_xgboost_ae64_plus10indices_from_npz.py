#!/usr/bin/env python3
"""
Train an XGBoost classifier on AE64 + 10 indices samples stored in NPZ.
...
Example:
python scripts/training/train_xgboost_ae64_plus10indices_from_npz.py \
  --data data/processed/training/ae64_plus10indices_samples_4upazila_2023_trainvaltest.npz \
  --outdir runs/xgb_ae64plus10idx_lr0.03_ne5000_md10_mcw3_sub0.8_col0.8_es200_v1 \
  --learning-rate 0.03 \
  --n-estimators 5000 \
  --max-depth 10 \
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--data``, ``--outdir``, ``--learning-rate``, ``--n-estimators``, ``--max-depth``, ``--min-child-weight``, ``--subsample``, ``--colsample-bytree``, ``--gamma``, ``--reg-alpha``, ``--reg-lambda``, ``--max-bin``, ``--early-stopping-rounds``, ``--eval-every``, ``--n-jobs``, ``--seed``, ``--tree-method``, ``--grow-policy``, ``--save-test-preds``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train XGBoost classifier on AE64 + 10 indices NPZ.")
    p.add_argument(
        "--data",
        type=Path,
