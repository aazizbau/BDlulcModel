...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--ae``, ``--year``, ``--interim-dir``, ``--upazilas``, ``--output``, ``--max-per-class``, ``--val-frac``, ``--test-frac``, ``--block-size-m``, ``--seed``, ``--label-nodata``, ``--ae-nodata``, ``--index-nodata``, ``--chunk``. Run the ``--help`` command below for required values, defaults, and accepted choices.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Extract AE64 + 10 spectral indices pixel samples with spatial train/val/test split + class balancing."
    )
    p.add_argument(
        "--ae",
        type=Path,
...
def main() -> None:
    ...
    for _, path in index_paths.items():
        if not path.exists():
    ...
    if all_Xte and all_yte:
        ...
        X_test = np.zeros((0, 74), dtype=np.float32)
