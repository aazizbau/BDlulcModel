...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The command-line interface exposes ``--ae``, ``--upazilas``, ``--output``, ``--max-per-class``, ``--val-frac``, ``--test-frac``, ``--block-size-m``, ``--seed``, ``--label-nodata``, ``--ae-nodata``, ``--chunk``. Run the ``--help`` command below for required values, defaults, and accepted choices.
