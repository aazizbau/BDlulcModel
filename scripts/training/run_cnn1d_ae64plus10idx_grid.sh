...
# ============================================================
# 1D-CNN grid search runner for AE64 + 10 indices classification
# ============================================================
#
# Runs 10 experiments and writes:
#   - per-run logs:
#       logs/cnn1d_ae64plus10idx_grid/<run_name>.log
#   - master comparison CSV:
#       runs/cnn1d_ae64plus10idx_master_runs.csv
#
# Usage:
#   chmod +x scripts/training/run_cnn1d_ae64plus10idx_grid.sh
...
TRAIN_SCRIPT="scripts/training/train_cnn1d_ae64_plus10indices_from_npz.py"
LOG_DIR="logs/cnn1d_ae64plus10idx_grid"
MASTER_CSV="runs/cnn1d_ae64plus10idx_master_runs.csv"
...
# - AE64 + 10 indices baseline is slightly stronger than AE64-only
# - best validation macro F1 reached about 0.6838
# - scheduler helped clearly after the plateau
# - overfitting is not severe, so search should test:
#     (a) mildly larger capacity
#     (b) slightly different receptive fields
#     (c) slightly different regularization
#     (d) learning-rate and batch-size stability
#
# Search logic:
# - vary width: baseline / wider
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h128_do02_lr1e3_bs4096_v1" \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.2 \
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k7-5-3_h128_do02_lr1e3_bs4096_v2" \
  --channels 32 64 128 \
  --kernels 7 5 3 \
  --head-dim 128 \
  --dropout 0.2 \
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h256_do02_lr1e3_bs4096_v3" \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 256 \
  --dropout 0.2 \
...
run_one "cnn1d_ae64plus10idx_c64-128-128_k5-3-3_h128_do02_lr1e3_bs4096_v4" \
  --channels 64 128 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.2 \
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h128_do03_lr1e3_bs4096_v5" \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.3 \
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h128_do015_lr1e3_bs4096_v6" \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.15 \
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h128_do02_lr7e4_bs4096_v7" \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.2 \
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h128_do02_lr15e4_bs4096_v8" \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.2 \
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h128_do02_lr1e3_bs8192_v9" \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.2 \
...
run_one "cnn1d_ae64plus10idx_c32-64-128_k5-3-3_h128_do02_lr1e3_bs4096_wd5e4_v10" \
  --channels 32 64 128 \
  --kernels 5 3 3 \
  --head-dim 128 \
  --dropout 0.2 \
...
master_csv = Path("runs/cnn1d_ae64plus10idx_master_runs.csv")
