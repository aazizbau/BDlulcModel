#!/usr/bin/env python3
"""
Train a ResNet-like MLP classifier on AE64 + 10 indices samples stored in NPZ.
...
- trains a PyTorch ResNet-like MLP classifier with weighted cross-entropy
...
Example:
python scripts/training/train_resmlp_ae64_plus10indices_from_npz.py \
  --data data/processed/training/ae64_plus10indices_samples_4upazila_2023_trainvaltest.npz \
  --outdir runs/resmlp_ae64plus10idx_h512-256_do02_lr1e3_bs4096_v1 \
  --hidden 512 256 \
  --dropout 0.2 \
  --batch-size 4096 \
  --epochs 100 \
...
- The residual MLP uses projection shortcuts automatically when block input/output
  dimensions differ.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train ResNet-like MLP classifier on AE64 + 10 indices NPZ.")
    p.add_argument(
        "--data",
        type=Path,
    ...
    p.add_argument(
        ...
        nargs="+",
        default=[512, 256],
        help="Residual block output sizes, e.g. --hidden 512 256",
    )
    p.add_argument(
        "--dropout",
        type=float,
        default=0.2,
        help="Dropout probability inside residual blocks (default: 0.2).",
    )
    p.add_argument(
        "--batch-size",
        type=int,
...
class ResMLPClassifier(nn.Module):
    def __init__(self, input_dim: int, hidden_dims: List[int], num_classes: int, dropout: float) -> None:
        ...
        for h in hidden_dims:
            blocks.append(ResMLPBlock(prev, h, dropout))
            prev = h
...
def evaluate(
    ...
    y_pred = np.concatenate(all_pred) if all_true else np.zeros((0,), dtype=np.int64)
    y_proba = np.concatenate(all_proba) if all_true else np.zeros((0, num_classes), dtype=np.float32)
