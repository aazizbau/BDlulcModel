#!/usr/bin/env python3
"""
Train a LightGBM classifier on AE64 + 10 indices samples stored in NPZ.
...
Example:
python scripts/training/train_lgbm_ae64_plus10indices_from_npz.py \
  --data data/processed/training/ae64_plus10indices_samples_4upazila_2023_trainvaltest.npz \
  --outdir runs/lgbm_ae64plus10idx_lr0.03_ne5000_nl127_mcs50_sub0.8_col0.8_es200_v1 \
  --learning-rate 0.03 \
  --n-estimators 5000 \
  --num-leaves 127 \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train LightGBM classifier on AE64 + 10 indices NPZ.")
    p.add_argument(
        "--data",
        type=Path,
...
def flatten_eval_history(eval_results: Dict[str, Dict[str, List[float]]]) -> List[Dict[str, float]]:
    ...
    n_rounds = 0
    for ds in datasets:
        for _, vals in eval_results[ds].items():
